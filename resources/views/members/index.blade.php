            ...
            <div class="h-64 overflow-y-auto space-y-3 border-t pt-2">
                @foreach ($members as $member)
                    <div class="flex items-center space-x-3 p-2 bg-gray-100 rounded">
                        ...
                        @endif
                        <div>
                            <h3 class="font-bold text-md"><a href="{{ route('profile.show', $member->id) }}"
                                    class="text-blue-500 hover:underline">{{ $member->username }}</a></h3>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="h-64 overflow-y-auto space-y-3 border-t pt-2">
                @foreach ($admins as $admin)
                    <div class="flex items-center space-x-3 p-2 bg-gray-100 rounded">
                        ...
                        @endif
                        <div>
                            <h3 class="font-bold text-md"><a href="{{ route('profile.show', $admin->id) }}"
                                    class="text-blue-500 hover:underline">{{ $admin->username }}</a></h3>
                        </div>
                    </div>
                @endforeach
            </div>
